...
rt_scaled <- function(n, df, mu, standard_dev) {
  x1 <- rt(n, df) # 1
  x2 <- x1 / sqrt(df / (df - 2)) # 2
...
for (i in 1:nrep) {
  ...
  beta <- matrix(0, p, 2)
  beta[idx_causal,1] <- rt_scaled(K_true, df, 0, sqrt(Sigma[1,1]))
  beta[idx_causal,2] <- rt_scaled(K_true, df, 0, sqrt(Sigma[2,2]))

  Beta1[, i] <- beta[, 1]/sqrt(var(beta[,1]))*sqrt(Sigma[1,1]/p*K_true)
  Beta2[, i] <- beta[, 2]/sqrt(var(beta[,2]))*sqrt(Sigma[2,2]/p*K_true)


  Phi1[, i] <- rt_scaled(p_chr,df,0,sqrt(omega1))
  Phi2[, i] <- rt_scaled(p_chr,df,0,sqrt(omega2))
...
write.table(Beta1, file = paste0("/import/home/share/mingxuan/fine_mapping/simulation/data/simu_loci_",i_loci,"_small_p",p,"_Beta1_t",df,"_new_Ktrue", K_true, "_Omega", h1_omega, "_", h2_omega, "_", rho_omega,
                                 "_enrich", enrich, "_rhoSigma", rho_sigma, "_nrep", nrep, ".txt"),
            col.names = T, row.names = F, quote = F, sep = "\t")
write.table(Beta2, file = paste0("/import/home/share/mingxuan/fine_mapping/simulation/data/simu_loci_",i_loci,"_small_p",p,"_Beta2_t",df,"_new_Ktrue", K_true, "_Omega", h1_omega, "_", h2_omega, "_", rho_omega,
                                 "_enrich", enrich, "_rhoSigma", rho_sigma, "_nrep", nrep, ".txt"),
            col.names = T, row.names = F, quote = F, sep = "\t")


write.table(Phi1, file = paste0("/import/home/share/mingxuan/fine_mapping/simulation/data/simu_loci_", i_loci, "_small_p", p, "_Phi1_t",df,"_chr22_Ktrue", K_true, "_Omega", h1_omega, "_", h2_omega, "_", rho_omega,
                                "_enrich", enrich, "_rhoSigma", rho_sigma, "_nrep", nrep, ".txt"),
            col.names = T, row.names = F, quote = F, sep = "\t")

write.table(Phi2, file = paste0("/import/home/share/mingxuan/fine_mapping/simulation/data/simu_loci_", i_loci, "_small_p", p, "_Phi2_t",df,"_chr22_Ktrue", K_true, "_Omega", h1_omega, "_", h2_omega, "_", rho_omega,
                                "_enrich", enrich, "_rhoSigma", rho_sigma, "_nrep", nrep, ".txt"),
            col.names = T, row.names = F, quote = F, sep = "\t")
